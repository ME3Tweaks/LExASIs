...
{
#pragma pack(push, 1)
    // Debugger frame information passed to LEX
    struct DebuggerFrame
    {
        ...
        BYTE* CodeBaseAddr;
        BYTE* Locals;
        FFrame::FOutParmRec* OutParms;
        DebuggerFrame* PreviousFrame;
        UFunction* NativeFunc;
        const char* NodePath;
        const wchar_t* FileName;
        uint16_t FileNameLength;
        uint16_t NodePathLength;
        uint16_t CurrentPosition;
    };

    // Message structure for WM_COPYDATA to LEX
    struct LexDebuggerMsg
    {
        DebuggerFrame* currentFrame;
        SFXNameEntry** NamePool;
        uint64_t msgLength;
        wchar_t msg[1024];
    };
#pragma pack(pop)

    // Pipe command identifiers
    enum class PipeCommand : unsigned char
    {
        AttachDebugger = 1,
        DetachDebugger = 2,
        BreakImmediate = 3,
    ...
    // Initialize the script debugger hooks
    void Initialize(::LESDK::Initializer& Init);

    // Get the debugger-specific pipe name
    ...
    void AttachDebugger();
    void DetachDebugger();
    void SendMsgToLEX(const std::wstring& msg, DebuggerFrame* stackFrame = nullptr);

    // The actual execution loop hook function
    void ExecutionLoop_hook(UObject* Context, FFrame* Stack, void* Result, tNative* const* GNatives);
